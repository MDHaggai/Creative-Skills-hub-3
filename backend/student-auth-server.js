...
const saltRounds = 10;
//const { Student, TempRegistration } = require('./models/studentModels');
...
db.on('connected', () => {
    console.log('successfully connected to student document in database');
});
db.on('error', (err) => {
...
//schema for temporal storage until email verification is complete
...
studentSchema.pre('save', async function(next) {
  ...
  // This middleware will now always hash the password, whether it's modified or not.
  try {
    if (this.password) {
...
async function sendVerificationEmail(email, verificationCode) {
  ...
  //connection to the mail service api
  try {
    const response = await axios({
      ...
      headers: {
        ...
        'X-RapidAPI-Key': process.env.RAPIDAPI_KEY, // Ensure your API key is stored in .env
...
app.post('/register_student', async (req, res) => {
  ...
  console.log(`Registering student with email: ${req.body.email} and generated code: ${verificationCode}`);

  // Hash the password before saving to TempRegistration
  try {
...
app.post('/verify_email', async (req, res) => {
  ...
  try {
    ...
    const { _id, verificationCode: _, ...userData } = tempReg.toObject();
    ...
    res.status(200).send({
      ...
      userType: 'student' // Or 'editor', 'client', etc., based on where the registration happened
    });
  } catch (error) {
    console.error(error);
